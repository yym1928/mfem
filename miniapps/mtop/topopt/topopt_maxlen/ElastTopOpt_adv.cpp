...
//
// Sample run:  mpirun -np 8 ./ElastTopOpt_adv
//              mpirun -np 8 ./ElastTopOpt_adv -r 6 -amax 0.3 -rt 2 -mi 500
...
void diagonalray(const Vector &x, Vector &v);
...
int main(int argc, char *argv[])
{
    ...
    int outflow_attr = mesh.bdr_attributes.Max() + 1;
    {
        for (int i = 0; i < mesh.GetNBE(); i++)
        {
            ...
            if (in_x_range && is_outflow)
                mesh.SetBdrAttribute(i, outflow_attr);
        }
        mesh.SetAttributes();
    }
    ...
    // 5b. Thickness design variables live on the outflow submesh.
    ...
    for(int a = 1; a <= outflow_attr; a++){
        if (a != clamp_attr) filter_solver.AddBoundaryID(a);
    }
    filter.Assemble();

    // 6c. Advection solver for the thickness measure, with the pseudo-transient
    //     time step set from the CFL condition.
    MaterialThicknessSolver advect(filter_fes, dgfes, *ray_cf, true);
    ...
    // Max-thickness constraint residual:  1/2 ∫_Gamma_out (rho_a − α)²
    ...
    for (; (it <= init_it) || (it <= max_it && iterationError > tol); it++)
    {
        ...
        // (5) advect rho~ along the ray field to get the thickness measure rho_a
        advect.SetRhs(rho_filter_tv);
        advect.FSolve();
        int ic = advect.GetSolver().GetIterCount();
        real_t thickres = adv_res.Eval();

        // (6) max-thickness constraint and gradient:  1/2 ∫(rho_a−α)² − ε ≤ 0
        //     dR/dalpha = (α − rho_a) on Gamma_out
        //     dR/drho   = M_fc^T N^T (rho_a − α)   via the adjoint advection solve
        Vector dGdrhoa;
